...
type_bitly = pyshorteners.Shortener(
    api_key=os.getenv('bitly_apiKey'))  #bitly api key
...
def sendSms():  #send sms function
    ...
    with open('phone.csv', mode='r') as phFile:
        ...
        for phno in phList:
            send_url = f"http://api.ininepal.com/api/index?username={os.getenv('sms_username')}&password={os.getenv('sms_pw')}&message={msg}&destination={phno[0]}&sender=SMS"
            print(send_url)
            print(phno[0] + " " + msg)
